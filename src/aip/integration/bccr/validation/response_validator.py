    ...
    def validate(self, payload: object) -> ValidationResult:
        if isinstance(payload, BCCRRequest):
            ...
            if not payload.indicator_codes:
                issues.append(
                    ValidationIssue(field="indicator_codes", message="indicator_codes is required")
                )
        ...
        if isinstance(payload, BCCRResponse):
            ...
            if not payload.indicator_code:
                response_issues.append(
                    ValidationIssue(field="indicator_code", message="indicator_code is required")
                )
            try:
                datetime.fromisoformat(payload.observation_date.replace("Z", "+00:00"))
            except ValueError:
                response_issues.append(
                    ValidationIssue(field="observation_date", message="observation_date is invalid")
                )
            if response_issues:
