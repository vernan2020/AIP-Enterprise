    ...
    def validate(self, payload: object) -> ValidationResult:
        if not isinstance(payload, SQLRequest):
            return ValidationResult(
                ok=False,
                issues=[ValidationIssue(field="payload", message="payload must be SQLRequest")],
            )
        ...
        if payload.parameters is None:
            issues.append(
                ValidationIssue(field="parameters", message="parameters must not be null")
            )
        if payload.page_size <= 0:
            issues.append(ValidationIssue(field="page_size", message="page_size must be positive"))

        return ValidationResult(
            ok=not issues, issues=issues, details={"query_name": payload.query_name}
        )
