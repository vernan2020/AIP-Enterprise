...
class NullProvider(Provider):
    ...
    def send(self, notification: Notification, *, timeout_seconds: float | None = None, cancellation_token: str | None = None) -> NotificationResult:
        return NotificationResult(notification_id=notification.notification_id, status=NotificationStatus.SENT, retries=0, provider=self.name, duration_seconds=0.0)
