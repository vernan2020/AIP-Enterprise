...
from aip.platform.notifications.models.alert import Alert
...
class NotificationEngine:
    def __init__(
        ...
        *,
        ...
        health: NotificationHealthMonitor | None = None,
    ...
    def process_event(
        self, event: dict[str, Any], *, rule: AlertRule | None = None, suppress: bool = False
    ) -> Alert | None:
        severity = (
        ...
        if self.deduplication_service.is_duplicate(event.get("event_type", "default")):
            ...
            return None
        alert = Alert(
            alert_id=str(hash(event.get("event_type", "default"))),
            title=event.get("message", "event"),
