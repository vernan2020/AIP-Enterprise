...
def bisection_solve(
    ...
    *,
    ...
    if f_lower == 0:
        return ConvergenceResult(root=lower, iterations=0, converged=True, residual=Decimal("0"), method="bisection")
    if f_upper == 0:
        return ConvergenceResult(root=upper, iterations=0, converged=True, residual=Decimal("0"), method="bisection")
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if abs(f_midpoint) <= tolerance:
            return ConvergenceResult(root=midpoint, iterations=iteration, converged=True, residual=abs(f_midpoint), method="bisection")
