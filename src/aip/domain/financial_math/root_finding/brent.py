    ...
    *,
    ...
    if f_lower == 0:
        return ConvergenceResult(
            root=lower, iterations=0, converged=True, residual=Decimal("0"), method="brent"
        )
    if f_upper == 0:
        return ConvergenceResult(
            root=upper, iterations=0, converged=True, residual=Decimal("0"), method="brent"
        )
    ...
    for iteration in range(1, max_iterations + 1):
        if abs(fa) <= tolerance:
            return ConvergenceResult(
                root=a, iterations=iteration, converged=True, residual=abs(fa), method="brent"
            )
        if abs(fb) <= tolerance:
            return ConvergenceResult(
                root=b, iterations=iteration, converged=True, residual=abs(fb), method="brent"
            )
        if fa * fb > 0:
        ...
        if abs(f_s) <= tolerance:
            return ConvergenceResult(
                root=s, iterations=iteration, converged=True, residual=abs(f_s), method="brent"
            )
        if fa * f_s <= 0:
            b, fb = s, f_s
        else:
