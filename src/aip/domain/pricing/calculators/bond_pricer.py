...
from aip.domain.financial_math import accrued_interest
...
from aip.domain.financial_math.bond_metrics import (
    convexity,
    dv01,
    macaulay_duration,
    modified_duration,
...
def price_bond(
    instrument: FinancialInstrument, *, valuation_date: date, market_yield: Decimal
) -> tuple[
    ...
    coupon_rate = (
        ...
        if getattr(instrument, "coupon_rate", None) is None
        else instrument.coupon_rate
    )
    ...
    cash_flows = [
        (Decimal("1"), instrument.nominal_value),
        (Decimal("2"), instrument.nominal_value),
    ]
