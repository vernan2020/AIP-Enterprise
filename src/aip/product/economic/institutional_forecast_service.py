...
class InstitutionalForecastService:
    ...
    def __init__(
        self,
        *,
        governance_service: ForecastGovernanceService | None = None,
        ...
        self._governance_service = governance_service or ForecastGovernanceService()
        ...
        *,
        ...
        if code not in self._SUPPORTED_INDICATORS:
            return self._unavailable(
                ...
                diagnostic=("Unsupported institutional " "forecast indicator"),
                reason_codes=("UNSUPPORTED_INDICATOR",),
            )
        ...
        temporal_metadata = self._resolve_temporal_metadata(
            dataset=dataset,
            indicator_code=code,
            series=series,
        )
        ...
        if governance.governance_model_name is None or governance.governance_model_family is None:
        ...
        model_result = governance.model_result(governance.governance_model_name)

        if model_result is None:
            return self._unavailable(
                indicator_code=code,
                horizon_months=(horizon_months),
                diagnostic=("Governance model result " "not found"),
                reason_codes=("GOVERNANCE_MODEL_MISSING",),
                governance=governance,
                series=series,
            )

        try:
            specification = self._specification_from_governance(
            ...
            return self._unavailable(
                ...
                reason_codes=("INVALID_GOVERNANCE_MODEL",),
                governance=governance,
                series=series,
            )

        estimation_warning_messages: list[str] = []
        ...
        try:
            with warnings.catch_warnings(record=True) as warning_records:
                ...
                forecasts = EconometricForecastService._forecast_multi_step(
                    series=series,
            ...
            for item in warning_records:
                if issubclass(
                    ...
                    estimation_warning_messages.append(
                        f"{item.category.__name__}: " f"{item.message}"
                    )
            ...
            return self._unavailable(
                ...
                horizon_months=(horizon_months),
                diagnostic=(
                    "Final governed model " "estimation failed: " f"{type(exc).__name__}: " f"{exc}"
                ),
                reason_codes=("FINAL_ESTIMATION_FAILED",),
                governance=governance,
                series=series,
            )
        ...
        if len(forecasts) != horizon_months:
            return self._unavailable(
                ...
                horizon_months=(horizon_months),
                diagnostic=(
                    "Governed forecast returned " "an unexpected number of " "projection points"
                ),
                reason_codes=("INVALID_FORECAST_LENGTH",),
                governance=governance,
                series=series,
            )

        if not all(math.isfinite(float(value)) for value in forecasts):
            return self._unavailable(
                indicator_code=code,
                horizon_months=(horizon_months),
                diagnostic=("Governed forecast contains " "non-finite values"),
                reason_codes=("NON_FINITE_FORECAST",),
                governance=governance,
                series=series,
            )
        ...
        horizon_metrics = tuple(
            InstitutionalHorizonMetric(
                ...
                mae=metric.mae,
                bias=metric.bias,
                naive_rmse=(metric.naive_rmse),
                relative_rmse=(metric.relative_rmse),
            )
            for metric in model_result.horizons
        )

        points: list[InstitutionalForecastPoint] = []

        z_score = EconometricForecastService._normal_quantile(self._confidence_level)
        ...
        for index, forecast_value in enumerate(
            ...
            horizon_rmse = self._rmse_for_projection_horizon(
            ...
            if horizon_rmse is not None and math.isfinite(horizon_rmse) and horizon_rmse >= 0.0:
                margin = z_score * horizon_rmse
        ...
        result_status = governance.status

        result_reason_codes = list(governance.reason_codes)

        result_warnings = list(governance.warnings)

        if estimation_warning_messages:
            result_warnings.extend(estimation_warning_messages)

            if "FINAL_ESTIMATION_WARNING" not in result_reason_codes:
                result_reason_codes.append("FINAL_ESTIMATION_WARNING")

            if result_status == "APPROVED":
                result_status = "APPROVED_WITH_WARNINGS"

        dynamic_status = None
        dynamic_ratio = None

        if governance.dynamic_stability is not None:
            dynamic_status = governance.dynamic_stability.status

            dynamic_ratio = governance.dynamic_stability.stability_ratio

        diagnostic_parts: list[str] = []

        if governance.diagnostic:
            diagnostic_parts.append(governance.diagnostic)
        ...
        return InstitutionalForecastResult(
            ...
            statistical_model_name=(governance.statistical_model_name),
            statistical_model_family=(governance.statistical_model_family),
            governance_model_name=(governance.governance_model_name),
            governance_model_family=(governance.governance_model_family),
            forecast_origin=(forecast_origin.date()),
            last_observed_value=(last_observed_value),
            historical_observations=len(series),
            horizon_months=(horizon_months),
            confidence_level=(self._confidence_level),
            weighted_relative_score=(governance.weighted_relative_score),
            improvement_vs_naive=(governance.improvement_vs_naive),
            materiality_threshold=(governance.materiality_threshold),
            horizon_metrics=(horizon_metrics),
            dynamic_stability_status=(dynamic_status),
            dynamic_stability_ratio=(dynamic_ratio),
            points=tuple(points),
            reason_codes=tuple(result_reason_codes),
            warnings=tuple(result_warnings),
            data_as_of_date=(temporal_metadata["data_as_of_date"]),
            forecast_origin_period=(temporal_metadata["forecast_origin_period"]),
            data_lag_days=(temporal_metadata["data_lag_days"]),
            data_lag_months=(temporal_metadata["data_lag_months"]),
            is_current_period=(temporal_metadata["is_current_period"]),
    ...
    def _specification_from_governance(
        *,
        ...
        if model_family == "AR":
            raw_lags = parameter_map.get("lags") or parameter_map.get("ar_lags")

            if raw_lags is None:
                if model_name.startswith("AR_"):
                    raw_lags = model_name.split(
                        "_",
                        1,
                    )[1]
                else:
                    raw_lags = EconometricForecastService._infer_ar_lags_from_name(model_name)

            return _SelectedSpecification(
                model_name=model_name,
                family="AR",
                ar_lags=int(raw_lags),
            )

        if model_family == "ARIMA":
            raw_order = parameter_map.get("order")

            if raw_order is not None:
                order = EconometricForecastService._parse_arima_order(raw_order)

            elif model_name.startswith("ARIMA_"):
                parts = model_name.replace(
                    "ARIMA_",
                    "",
                    1,
                ).split("_")
                ...
                if len(parts) != 3:
                    raise ValueError("Invalid governed " "ARIMA model name")

                order = tuple(int(value) for value in parts)

            else:
                order = EconometricForecastService._infer_arima_order_from_name(model_name)
        ...
        raise ValueError("Unsupported governed model family: " f"{model_family}")
    ...
    def forecast_all_calendar_aligned(
        ...
        *,
        ...
        dataset_as_of_date = dataset.as_of_date

        if dataset_as_of_date is None:
            raise ValueError("Econometric dataset does not have " "an as-of date.")

        scenario_calendar = build_scenario_calendar(
            dataset_as_of_date=(dataset_as_of_date),
            horizon_months=(horizon_months),
        )

        scenario_periods = set(scenario_calendar)

        baseline_results = self.forecast_all(
            dataset,
            horizon_months=(horizon_months),
        )

        aligned_results: list[InstitutionalForecastResult] = []

        for baseline in baseline_results:
            if not baseline.points:
                aligned_results.append(baseline)
                continue

            forecast_origin_period = baseline.forecast_origin_period
            ...
            required_horizon = required_projection_horizon(
                forecast_origin_period=(forecast_origin_period),
                scenario_calendar=(scenario_calendar),
            )

            candidate = baseline

            if required_horizon > horizon_months:
            ...
            selected_points = tuple(
                point for point in candidate.points if (point.target_period in scenario_periods)
            )

            if len(selected_points) != horizon_months:
                raise ValueError(
            ...
            selected_periods = tuple(point.target_period for point in selected_points)

            if selected_periods != scenario_calendar:
                raise ValueError(
            ...
            aligned_results.append(
                replace(
                    candidate,
                    horizon_months=(horizon_months),
                    points=(institutional_points),
                )
            )

        return tuple(aligned_results)
    ...
    def _rmse_for_projection_horizon(
        *,
        ...
        ordered = sorted(
            (metric for metric in model_result.horizons if metric.rmse is not None),
            key=lambda item: (item.horizon_months),
        )
        ...
        for metric in ordered:
            if horizon <= metric.horizon_months:
                return float(metric.rmse)

        return float(ordered[-1].rmse)
    ...
    def _resolve_temporal_metadata(
        *,
        ...
        forecast_origin_period = series.index[-1].date()

        point = dataset.latest_data_point(indicator_code)

        if point is not None:
            data_as_of_date = point.observation_date
        else:
            # Compatibilidad con datasets sintéticos
            # construidos directamente en tests.
            data_as_of_date = forecast_origin_period

        dataset_as_of_date = dataset.as_of_date or data_as_of_date

        lag_days = max(
            0,
            (dataset_as_of_date - data_as_of_date).days,
        )

        lag_months = max(
            0,
            (dataset_as_of_date.year - data_as_of_date.year) * 12
            + (dataset_as_of_date.month - data_as_of_date.month),
        )

        is_current_period = (
            data_as_of_date.year == dataset_as_of_date.year
            and data_as_of_date.month == dataset_as_of_date.month
        )
    ...
    def _from_unavailable_governance(
        ...
        *,
        ...
        return self._unavailable(
            indicator_code=(governance.indicator_code),
            horizon_months=(horizon_months),
            diagnostic=(governance.diagnostic or ("Forecast governance " "is unavailable")),
            reason_codes=(governance.reason_codes or ("GOVERNANCE_UNAVAILABLE",)),
            governance=governance,
            series=series,
        )
    ...
    def _unavailable(
        ...
        *,
        ...
        governance: ForecastGovernanceResult | None = None,
        series=None,
    ) -> InstitutionalForecastResult:
        ...
        return InstitutionalForecastResult(
            ...
            statistical_model_name=(
                governance.statistical_model_name if governance is not None else None
            ),
            statistical_model_family=(
                governance.statistical_model_family if governance is not None else None
            ),
            governance_model_name=(
                governance.governance_model_name if governance is not None else None
            ),
            governance_model_family=(
                governance.governance_model_family if governance is not None else None
            ),
            ...
            weighted_relative_score=(
                governance.weighted_relative_score if governance is not None else None
            ),
            improvement_vs_naive=(
                governance.improvement_vs_naive if governance is not None else None
            ),
            materiality_threshold=(
                governance.materiality_threshold if governance is not None else 0.0
            ),
            horizon_metrics=(),
            dynamic_stability_status=(
                governance.dynamic_stability.status
                if (governance is not None and governance.dynamic_stability is not None)
                else None
            ),
            dynamic_stability_ratio=(
                governance.dynamic_stability.stability_ratio
                if (governance is not None and governance.dynamic_stability is not None)
                else None
            ),
            points=(),
