...
class AdvancedMacroForecastingService:
    ...
    The service deliberately remains separate from the approved institutional
    scenario.  It produces a candidate forecast, model diagnostics and an
    ensemble that can later be reviewed and promoted through the existing
    scenario-governance workflow.

    Leakage controls:
    - rolling origins use only observations available at each origin;
    - direct multivariate models train only on targets known by the origin;
    - no interpolation, forward-fill or back-fill is performed;
    - contemporaneous driver values are used only when observed at the origin.
    """
    ...
    def evaluate(
        ...
        *,
        ...
        naive_metrics = raw_results.get("NAIVE", ())
        naive_by_horizon = {item.horizon: item for item in naive_metrics}
        ...
        for specification in self._registry.enabled():
            ...
            for metric in raw:
                ...
                relative = self._relative_rmse(metric.rmse, naive.rmse if naive else None)
                converted.append(
                    MultiHorizonMetric(
                ...
                weight = dict(self._horizon_weights).get(metric.horizon, 0.0)
                if (
            ...
            if diagnostics.get(specification.name, "") == "OPTIONAL_DEPENDENCY_UNAVAILABLE":
                status = "UNAVAILABLE"
            elif not converted:
                status = "FAILED"
            elif score is None:
        ...
        champion = self._select_champion(tuple(model_results))
        if champion is None:
            return AdvancedIndicatorForecastResult(
                indicator_code=code,
                status="FAILED",
        ...
        weights = self._ensemble_weights(tuple(model_results), champion)
        if not weights:
            return AdvancedIndicatorForecastResult(
                indicator_code=code,
                status="FAILED",
    ...
    def _backtest_model(
        ...
        for horizon, _weight in self._horizon_weights:
            try:
                metric = self._backtest_horizon(frame, target, specification, horizon)
            except ImportError:
    ...
    def _backtest_horizon(
        ...
        for origin in range(first_origin, last_origin + 1):
            ...
            try:
                ...
                raise
            except (ValueError, TypeError, np.linalg.LinAlgError, RuntimeError, OverflowError):
                failures += 1
                continue
    ...
    def _forecast_direct(
        ...
        if len(y_train) < max(24, self._minimum_training_observations - 6):
    ...
    def _direct_training_data(
        ...
        x_predict = self._feature_vector(frame, target, origin)
        training_x: list[np.ndarray] = []
        training_y: list[float] = []
        max_feature_lag = max(self._FEATURE_LAGS)
        ...
        for feature_origin in range(max_feature_lag, last_feature_origin + 1):
            ...
            if pd.isna(target_value):
                continue
            try:
                features = self._feature_vector(frame, target, feature_origin)
            except ValueError:
                continue
    ...
    def _feature_vector(self, frame: pd.DataFrame, target: str, position: int) -> np.ndarray:
        values: list[float] = []
        for column in frame.columns:
            for lag in self._FEATURE_LAGS:
                source_position = position - lag
                if source_position < 0:
                    raise ValueError("Insufficient lag history")
                value = frame[column].iloc[source_position]
                if pd.isna(value):
                    raise ValueError("Missing lagged feature")
                values.append(float(value))
        current = frame[target].iloc[position]
        lag_1 = frame[target].iloc[position - 1]
        lag_3 = frame[target].iloc[position - 3]
        lag_12 = frame[target].iloc[position - 12]
        if any(pd.isna(value) for value in (current, lag_1, lag_3, lag_12)):
            raise ValueError("Missing target change feature")
        values.extend(
            (
                float(current - lag_1),
                float(current - lag_3),
                float(current - lag_12),
            )
        )
    ...
    def _ols_predict(
        ...
        return float(np.concatenate(([1.0], x_predict)) @ beta)
    ...
    def _forecast_ensemble(
        ...
        for horizon in range(1, forecast_horizon + 1):
            ...
            for _value, raw_weight, model_name in model_values:
                model_result = results_by_name[model_name]
                rmse = self._interpolated_rmse(model_result, horizon)
                if rmse is not None:
            ...
            lower_80 = point_forecast - 1.2815515655446004 * sigma
            upper_80 = point_forecast + 1.2815515655446004 * sigma
            lower_95 = point_forecast - 1.959963984540054 * sigma
            upper_95 = point_forecast + 1.959963984540054 * sigma
    ...
    def _confidence_score(
        *,
        ...
        improvement = max(0.0, min(1.0, (champion.improvement_vs_naive or 0.0) / 0.20))
