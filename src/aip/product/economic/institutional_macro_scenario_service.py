...
class InstitutionalMacroScenarioService:
    ...
    def __init__(
        ...
        *,
        ...
        self._forecast_service = forecast_service or InstitutionalForecastService(
            maximum_horizon=24
        )
    ...
    def create_draft(
        ...
        *,
        ...
        if dataset.as_of_date is None:
            raise ValueError("Dataset does not contain a real as-of date")
        ...
        if normalized_type not in {
            ...
            raise ValueError("Unsupported scenario_type: " f"{scenario_type}")
        ...
        if scenario_id is None:
            scenario_id = "MACRO-" + uuid4().hex.upper()
        ...
        results = self._forecast_service.forecast_all_calendar_aligned(
            dataset,
        ...
        version = self._repository.next_version(scenario_id)

        indicators = tuple(self._map_indicator(result) for result in results)
        ...
        self._repository.save(scenario)
    ...
    def _validate_results(
        ...
        *,
        ...
        codes = {result.indicator_code for result in results}

        missing = self._EXPECTED_INDICATORS - codes

        unexpected = codes - self._EXPECTED_INDICATORS

        if missing:
            raise ValueError(
                "Institutional forecast is missing "
                "required indicators: " + ", ".join(sorted(missing))
            )

        if unexpected:
            raise ValueError(
                "Institutional forecast contains "
                "unexpected indicators: " + ", ".join(sorted(unexpected))
            )

        if len(results) != len(self._EXPECTED_INDICATORS):
            raise ValueError("Institutional forecast contains " "duplicate indicators")

        for result in results:
            if result.status == "UNAVAILABLE":
                continue

            if len(result.points) != horizon_months:
                raise ValueError(
            ...
            if horizons != expected_horizons:
                raise ValueError("Non-contiguous forecast horizons for " f"{result.indicator_code}")
    ...
    def _map_indicator(
        ...
        return InstitutionalMacroScenarioIndicator(
            indicator_code=(result.indicator_code),
            statistical_model_name=(result.statistical_model_name),
            statistical_model_family=(result.statistical_model_family),
            governance_model_name=(result.governance_model_name),
            governance_model_family=(result.governance_model_family),
            institutional_status=(result.status),
            data_as_of_date=(result.data_as_of_date),
            forecast_origin_period=(result.forecast_origin_period),
            last_observed_value=(result.last_observed_value),
            historical_observations=(result.historical_observations),
            weighted_relative_score=(result.weighted_relative_score),
            improvement_vs_naive=(result.improvement_vs_naive),
            dynamic_stability_status=(result.dynamic_stability_status),
            dynamic_stability_ratio=(result.dynamic_stability_ratio),
            data_lag_days=(result.data_lag_days),
            data_lag_months=(result.data_lag_months),
            is_current_period=(result.is_current_period),
            approved_for_base_scenario=(result.approved_for_base_scenario),
