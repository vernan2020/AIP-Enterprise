...
class ConfiguredDependencyComposition:
    ...
    def compose(self, container: Container) -> Container:
        ...
        container.register_factory(
            InitialLoadWorkflow,
            lambda current: InitialLoadWorkflow(
                self._config,
                portfolio_provider=current.resolve(PortfolioDataProvider),
                market_provider=current.resolve(MarketDataProvider),
                liquidity_provider=current.resolve(LiquidityDataProvider),
                health_provider=current.resolve(SourceHealthProvider),
            ),
        )
        container.register_factory(
            RefreshAllWorkflow,
            lambda current: RefreshAllWorkflow(
                self._config,
                portfolio_provider=current.resolve(PortfolioDataProvider),
                market_provider=current.resolve(MarketDataProvider),
                liquidity_provider=current.resolve(LiquidityDataProvider),
                health_provider=current.resolve(SourceHealthProvider),
                valuation_date_context=current.resolve(ValuationDateContext),
            ),
        )
        container.register_factory(
            ExecutiveRefreshWorkflow,
            lambda current: ExecutiveRefreshWorkflow(
                self._config,
                valuation_date_context=current.resolve(ValuationDateContext),
            ),
        )
    ...
    def _validate_required_services(container: Container) -> None:
        """Fail fast when any institutional runtime registration is incomplete."""
        required_services = (
            SourceHealthProvider,
            PortfolioDataProvider,
            MarketDataProvider,
