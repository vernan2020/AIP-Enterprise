...
class ConfiguredPortfolioProvider:
    ...
    def get_portfolio(self) -> dict[str, Any]:
        ...
        vector_records_available_for_matching = len(
            [
        ...
        weighted_yield = PortfolioCalculationService.weighted_average_effective_yield(
        ...
        print(
            f"[instrumentation] STEP2 len(price_vector.positions)={len(price_vector_positions)}",
            flush=True,
        )
        self._emit_b180429_trace("STEP2", price_vector_positions)
        print(
            f"[instrumentation] ConfiguredPortfolioProvider returning with vector_records_available_for_matching={vector_records_available_for_matching}",
            flush=True,
        )
        return {
            ...
            or self._config.data_cutoff_date.isoformat(),
    ...
    def _resolve_vector_directories(self, year_dir: Path) -> list[Path]:
        ...
        for child in sorted(year_dir.iterdir(), key=lambda item: item.name):
            if not child.is_dir():
                continue
            normalized_child_name = self._normalize_alias(child.name).casefold()
            if normalized_child_name not in normalized_aliases:
                continue
            normalized_path = str(child).casefold()
            if normalized_path in seen_candidates:
                continue
            seen_candidates.add(normalized_path)
            alias_candidates.append(child)
    ...
    def _read_vector_result(
        ...
        print(
            f"[instrumentation] STEP1 accepted_records_count={read_result.accepted_count}",
            flush=True,
        )
        self._emit_b180429_trace(
            "STEP1",
            [record for record in read_result.records if hasattr(record, "issuer")],
            use_raw=True,
        )
        base_result.update(
            {
    ...
    def _emit_b180429_trace(
        self, step: str, positions: list[Any], *, use_raw: bool = False
    ) -> None:
            ...
            if not isinstance(item, dict):
                if use_raw and hasattr(item, "issuer"):
                    ...
                    if self._normalize_text(series) == "b180429":
                        print(
                            f"[instrumentation] {step} issuer={issuer} product_code={product_code} series={series} maturity_date={self._serialize_date(maturity)}",
                            flush=True,
                        )
                continue
            issuer = item.get("issuer", "")
            product_code = item.get("instrument_type_or_mnemonic", "") or item.get(
                "product_code", ""
            )
