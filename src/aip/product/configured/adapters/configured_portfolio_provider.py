...
class ConfiguredPortfolioProvider:
    ...
    def get_portfolio(self) -> dict[str, Any]:
        ...
        if positions:
            ...
            valuation_date_raw = portfolio_master.get("valuation_date") or self._current_cutoff_date().isoformat()
            valuation_date = date.fromisoformat(valuation_date_raw) if isinstance(valuation_date_raw, str) else valuation_date_raw

            for position in positions:
                duration_result = PortfolioDurationService.calculate(position, valuation_date)
                position["modified_duration"] = float(duration_result.modified_duration) if duration_result.modified_duration is not None else None
                position["duration_method"] = duration_result.method
                position["duration_source"] = duration_result.source
                position["next_repricing_date"] = duration_result.next_repricing_date.isoformat() if duration_result.next_repricing_date is not None else None
        ...
        hqla_percent = (hqla_value_crc / market_value * Decimal("100")) if market_value > 0 else Decimal("0")
    ...
    def _apply_position_payload_fields(self, position: dict[str, Any]) -> dict[str, Any]:
        ...
        vector_record = position.get("vector_record") if isinstance(position.get("vector_record"), dict) else {}
