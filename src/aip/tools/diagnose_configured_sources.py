...
def _trace_security(payload: dict[str, Any], *, trace_security: str | None) -> None:
    ...
    vector_matches = [
        ...
        if str(record.get("series_or_security_code", "")).upper() == trace_security.upper()
    ]
    print("STEP 1")
    ...
    print("STEP 4")
    print(
        f"series_maturity: {matching.get('master_key_sample', {}).get('series_maturity') if isinstance(matching.get('master_key_sample'), dict) else None}"
    )
    print(
        f"issuer_product_maturity: {matching.get('master_key_sample', {}).get('issuer_product_maturity') if isinstance(matching.get('master_key_sample'), dict) else None}"
    )
    print("STEP 5")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"Header row: {payload['portfolio_master'].get('diagnostics', {}).get('header_row', 'N/A')}"
    )
    print(
        f"Rows read: {payload['portfolio_master'].get('diagnostics', {}).get('rows_read', 'N/A')}"
    )
    print(
        f"Rows accepted: {payload['portfolio_master'].get('diagnostics', {}).get('rows_accepted', 'N/A')}"
    )
    print(f"Rows rejected: {row_count}")
    print(f"Mapped columns: {len(payload['portfolio_master'].get('detected_column_mapping', {}))}")
    print(
        f"ISIN count: {sum(1 for position in payload.get('positions', []) if position.get('isin'))}"
    )
    warnings = payload["portfolio_master"].get("warnings", ())
    print(f"Warnings: {', '.join(warnings[:3]) if warnings else 'None'}")
    first_rejected_row = (
        payload["portfolio_master"].get("diagnostics", {}).get("first_rejected_row")
    )
    ...
    print(
        f"ISIN count: {len(payload['price_vector'].get('diagnostics', {}).get('trace', {}).get('isin_found', []))}"
    )
    rejected_reason_counts = (
        payload["price_vector"]
        .get("diagnostics", {})
        .get("trace", {})
        .get("rejected_reason_counts", {})
    )
    if rejected_reason_counts:
        print("Rejected reasons:")
        for reason, count in sorted(
            rejected_reason_counts.items(), key=lambda item: (-item[1], item[0])
        ):
            print(f"  - {reason}: {count}")
    else:
        print("Rejected reasons: None")
    accepted_records = (
        payload["price_vector"].get("diagnostics", {}).get("trace", {}).get("accepted_records", [])
    )
    if accepted_records:
    ...
    if not match_summary:
        match_summary = (
            payload.get("diagnostics", {}).get("portfolio_master", {}).get("matching", {})
        )
    ...
    print(
        f"Matched by issuer/product/maturity: {match_summary.get('issuer_product_maturity_matches', 0)}"
    )
    ...
    print(
        f"vector_records_available_for_matching: {payload['price_vector'].get('accepted_count', 0)}"
    )
