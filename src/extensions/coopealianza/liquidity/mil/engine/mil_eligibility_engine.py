...
class MilEligibilityEngine:
    ...
    def evaluate(self, request: MilRequest) -> MilResult:
        ...
        for asset in assets:
            status, blocking_factors, warning_factors, haircut, adjusted_value, reasoning = (
                self._evaluate_asset(asset, config, policy_context)
            )
            ...
            position = MilPositionResult(
                ...
                evidence=({"position_id": asset.position_id, "reason": reasoning},),
            )
            positions.append(position)

            self._accumulate_capacity(
                capacity_values, asset, adjusted_value, status, policy_context
            )

        capacity_values["total_market_value_evaluated"] = total_market_value
        capacity_values["eligible_market_value"] = sum(
            (
                p.market_value
                for p in positions
                if p.eligibility_status == MilEligibilityStatus.ELIGIBLE
            ),
            Decimal("0"),
        )
        capacity_values["conditionally_eligible_market_value"] = sum(
            (
                p.market_value
                for p in positions
                if p.eligibility_status == MilEligibilityStatus.CONDITIONALLY_ELIGIBLE
            ),
            Decimal("0"),
        )
        capacity_values["ineligible_market_value"] = sum(
            (
                p.market_value
                for p in positions
                if p.eligibility_status == MilEligibilityStatus.NOT_ELIGIBLE
            ),
            Decimal("0"),
        )
        capacity_values["unknown_market_value"] = sum(
            (
                p.market_value
                for p in positions
                if p.eligibility_status == MilEligibilityStatus.UNKNOWN
            ),
            Decimal("0"),
        )
        ...
        capacity_values["total_potential_collateral_capacity"] = (
            eligible_capacity + conditional_capacity
        )
    ...
    def _evaluate_asset(
        self, asset: Any, config: MilPolicyConfig, policy_context: dict[str, Any]
    ) -> tuple[MilEligibilityStatus, tuple[str, ...], tuple[str, ...], Decimal, Decimal, str]:
        ...
        if (
            config.reserve_liquidity_treatment == "conditional"
            and asset.reserve_liquidity_status.lower() == "reserve"
        ):
        ...
        if blocking_factors:
            return (
                MilEligibilityStatus.NOT_ELIGIBLE,
                tuple(blocking_factors),
                tuple(warning_factors),
                haircut,
                adjusted_value,
                "; ".join(blocking_factors),
            )
        ...
        if warning_factors:
            return (
                ...
                haircut,
                adjusted_value,
                "; ".join(warning_factors),
            )

        return (
            MilEligibilityStatus.ELIGIBLE,
            tuple(blocking_factors),
            tuple(warning_factors),
            haircut,
            adjusted_value,
    ...
    def _accumulate_capacity(
        self,
        capacity_values: dict[str, Any],
        asset: Any,
        adjusted_value: Decimal,
        status: MilEligibilityStatus,
        policy_context: dict[str, Any],
    ) -> None:
        if status == MilEligibilityStatus.ELIGIBLE:
            capacity_values["eligible_adjusted_collateral_value"] = (
                capacity_values["eligible_adjusted_collateral_value"] + adjusted_value
            )
        elif status == MilEligibilityStatus.CONDITIONALLY_ELIGIBLE:
            capacity_values["conditional_adjusted_collateral_value"] = (
                capacity_values["conditional_adjusted_collateral_value"] + adjusted_value
            )
        ...
        capacity_values["capacity_by_issuer"][asset.issuer] = (
            capacity_values["capacity_by_issuer"].get(asset.issuer, Decimal("0")) + adjusted_value
        )
        capacity_values["capacity_by_currency"][asset.currency] = (
            capacity_values["capacity_by_currency"].get(asset.currency, Decimal("0"))
            + adjusted_value
        )
        capacity_values["capacity_by_maturity_band"][maturity_band] = (
            capacity_values["capacity_by_maturity_band"].get(maturity_band, Decimal("0"))
            + adjusted_value
        )
        if status == MilEligibilityStatus.ELIGIBLE:
            capacity_values["capacity_by_classification"][asset.classification] = (
                capacity_values["capacity_by_classification"].get(
                    asset.classification, Decimal("0")
                )
                + adjusted_value
            )
