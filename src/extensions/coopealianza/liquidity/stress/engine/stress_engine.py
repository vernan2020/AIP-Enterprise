...
class StressEngine:
    ...
    def evaluate(self, request: StressRequest) -> StressResult:
        ...
        scenario_results = [
            self._evaluate_scenario(
                scenario, request.gap_result, request.projection_result, scenario_lookup
            )
        ...
        return StressResult(
            ...
            stressed_parameters={
                key: value
                for scenario in scenarios
                for key, value in self._scenario_parameters(scenario).items()
            },
            policy_references=tuple(
                {reference for scenario in scenarios for reference in scenario.policy_references}
            ),
            warnings=tuple({warning for scenario in scenarios for warning in scenario.warnings}),
            affected_assets=tuple(
                {asset for scenario in scenarios for asset in scenario.affected_assets}
            ),
            affected_buckets=tuple(
                {bucket for scenario in scenarios for bucket in scenario.affected_buckets}
            ),
    ...
    def _evaluate_scenario(
        ...
        scenario_lookup: dict[str, StressScenarioConfig],
    ) -> StressScenarioResult:
        if scenario.scenario_type == "combined":
            return self._evaluate_combined_scenario(
                scenario, gap_result, projection_result, scenario_lookup
            )
        ...
        if scenario.scenario_type == "deposit_runoff":
            ...
            stressed_gap = gap_result.net_gap + (
                gap_result.net_gap * (Decimal("1") - scenario.collateral_multiplier)
            )
            ...
            stressed_gap = gap_result.net_gap + (
                gap_result.net_gap * scenario.market_value_multiplier
            )
            stressed_outflow = gap_result.gross_outflow * (Decimal("1") + scenario.severity)
            stressed_inflow = gap_result.gross_inflow * (
                Decimal("1") - scenario.market_value_multiplier
            )
    ...
    def _evaluate_combined_scenario(
        self,
        scenario: StressScenarioConfig,
        gap_result: GapResult,
        projection_result: ProjectionResult,
        ...
        for scenario_id in sorted(scenario.combined_scenario_ids):
            ...
            combined_results.append(
                self._evaluate_scenario(referenced, gap_result, projection_result, scenario_lookup)
            )
        ...
        return StressScenarioResult(
            ...
            assumptions=scenario.assumptions,
            stressed_parameters={
                "combined_scenario_ids": Decimal(len(scenario.combined_scenario_ids))
            },
            policy_references=scenario.policy_references,
            warnings=scenario.warnings,
            affected_assets=scenario.affected_assets,
    ...
    def _coerce_config(self, config: Any) -> StressPolicyConfig:
        ...
        raise StressConfigurationError(
            "Stress configuration must be a StressPolicyConfig or mapping"
        )
