...
class TreasuryDecisionEngine:
    ...
    def evaluate(self, request: TreasuryDecisionRequest) -> TreasuryDecisionResult:
        ...
        if not request.policy_results and not request.configuration:
            raise TreasuryDecisionEvaluationError(
                "At least one policy result or configuration is required"
            )
        ...
        return TreasuryDecisionResult(
            ...
            calculation_identifier=request.calculation_id
            or f"treasury-decision-{request.portfolio_reference}",
            correlation_id=request.correlation_id,
        )

    def _build_recommendations(
        self, request: TreasuryDecisionRequest, config: DecisionConfig, review: bool
        ...
        def is_enabled(recommendation_type: RecommendationType) -> bool:
            if not enabled_types:
                return True
            return (
                recommendation_type.value.lower() in enabled_types
                or recommendation_type.name.lower() in enabled_types
            )

        if (
            is_enabled(RecommendationType.BUY)
            and request.relative_value_result is not None
            ...
            recommendations.append(
                self._make_recommendation(
                    ...
                    PriorityLevel.HIGH,
                    ImpactMetrics(
                        liquidity_gap_impact=Decimal("0.20"),
                        market_value_exposure=Decimal("100000"),
                    ),
                    (RecommendationType.SELL, RecommendationType.MONITOR),
                )
            )
        if (
            is_enabled(RecommendationType.SELL)
            and request.relative_value_result is not None
            and base_score <= sell_threshold
        ):
            recommendations.append(
                self._make_recommendation(
                    request,
                    ...
                    ImpactMetrics(
                        liquidity_gap_impact=Decimal("-0.10"),
                        market_value_exposure=Decimal("-50000"),
                    ),
                    (RecommendationType.BUY, RecommendationType.NO_ACTION),
                )
            )
        if (
            is_enabled(RecommendationType.USE_AS_COLLATERAL)
            and request.mil_result is not None
            and request.mil_result.status_counts.get("eligible")
        ):
            recommendations.append(
                self._make_recommendation(
                    request,
                    RecommendationType.USE_AS_COLLATERAL,
                    base_score,
                    PriorityLevel.MEDIUM,
                    ImpactMetrics(
                        collateral_capacity_impact=Decimal("0.10"),
                        mil_capacity_impact=Decimal("0.05"),
                    ),
                    (RecommendationType.DO_NOT_USE_AS_COLLATERAL,),
                )
            )
        if (
            is_enabled(RecommendationType.DO_NOT_USE_AS_COLLATERAL)
            and request.mil_result is not None
            and request.mil_result.status_counts.get("ineligible")
        ):
            recommendations.append(
                self._make_recommendation(
                    request,
        ...
        if is_enabled(RecommendationType.REDUCE_CONCENTRATION) and self._has_concentration_breach(
            request, config
        ):
            recommendations.append(
                self._make_recommendation(
                    ...
                    base_score,
                    PriorityLevel.HIGH,
                    ImpactMetrics(concentration_impact=Decimal("0.10")),
                    (RecommendationType.NO_ACTION,),
                )
            )
        if (
            is_enabled(RecommendationType.IMPROVE_LIQUIDITY)
            and request.gap_result is not None
            and request.gap_result.net_gap < 0
        ):
            recommendations.append(
                self._make_recommendation(
                    request,
                    RecommendationType.IMPROVE_LIQUIDITY,
                    base_score,
                    PriorityLevel.CRITICAL,
                    ImpactMetrics(liquidity_gap_impact=Decimal("0.15")),
                    (RecommendationType.NO_ACTION,),
                )
            )
        if (
            is_enabled(RecommendationType.LIMIT_EXCESS_RISK)
            and request.stress_result is not None
            and request.stress_result.summary.get("max_effect", Decimal("0")) > Decimal("0")
        ):
            recommendations.append(
                self._make_recommendation(
                    request,
                    RecommendationType.LIMIT_EXCESS_RISK,
                    base_score,
                    PriorityLevel.HIGH,
                    ImpactMetrics(stress_resilience_impact=Decimal("0.10")),
                    (RecommendationType.NO_ACTION,),
                )
            )
        if (
            is_enabled(RecommendationType.HOLD)
            and request.relative_value_result is not None
            and request.relative_value_result.relative_value_score <= sell_threshold
        ):
            recommendations.append(
                self._make_recommendation(
                    request,
                    ...
                    PriorityLevel.MEDIUM,
                    ImpactMetrics(policy_compliance_impact=Decimal("0.01")),
    ...
    def _make_recommendation(
        ...
        rejected: tuple[RecommendationType, ...],
    ) -> Recommendation:
        return Recommendation(
            ...
            policy_references=tuple(
                ref.identifier for result in request.policy_results for ref in result.references
            ),
    ...
    def _dedupe_recommendations(
        self, recommendations: tuple[Recommendation, ...]
    ) -> tuple[Recommendation, ...]:
    ...
    def _ensure_no_conflicts(self, recommendations: tuple[Recommendation, ...]) -> None:
        conflicting = [
            recommendation
            for recommendation in recommendations
            if recommendation.recommendation in {RecommendationType.BUY, RecommendationType.SELL}
        ]
    ...
    def _build_groups(
        ...
        groups = [
            RecommendationGroup(
                ...
                recommendations=tuple(item for item in ordered if item.priority == priority),
    ...
    def _score(self, request: TreasuryDecisionRequest) -> Decimal:
        ...
        if request.stress_result is not None:
            stress_score = Decimal(
                str(request.stress_result.summary.get("max_effect", Decimal("0")))
            )
        blended_score = (relative_value_score * Decimal("0.95")) + (
            (stress_score / Decimal("10000")) * Decimal("0.05")
        )
    ...
    def _confidence(self, recommendation_type: RecommendationType) -> Decimal:
        return {
            RecommendationType.ACCUMULATE: Decimal("0.90"),
            RecommendationType.BUY: Decimal("0.90"),
            RecommendationType.SELL: Decimal("0.85"),
            RecommendationType.HOLD: Decimal("0.80"),
            RecommendationType.USE_AS_COLLATERAL: Decimal("0.82"),
            RecommendationType.DO_NOT_USE_AS_COLLATERAL: Decimal("0.81"),
            RecommendationType.REDUCE_CONCENTRATION: Decimal("0.83"),
            RecommendationType.IMPROVE_LIQUIDITY: Decimal("0.84"),
            RecommendationType.LIMIT_EXCESS_RISK: Decimal("0.81"),
            RecommendationType.MONITOR: Decimal("0.70"),
            RecommendationType.NO_ACTION: Decimal("0.65"),
        }[recommendation_type]
    ...
    def _rationale(self, recommendation_type: RecommendationType) -> tuple[str, ...]:
        return {
            RecommendationType.ACCUMULATE: ("relative_value",),
            RecommendationType.BUY: ("relative_value",),
            RecommendationType.SELL: ("relative_value", "risk"),
            RecommendationType.HOLD: ("stability",),
            RecommendationType.USE_AS_COLLATERAL: ("mil_eligibility",),
            RecommendationType.DO_NOT_USE_AS_COLLATERAL: ("mil_ineligibility",),
            RecommendationType.REDUCE_CONCENTRATION: ("concentration",),
            RecommendationType.IMPROVE_LIQUIDITY: ("liquidity_gap",),
            RecommendationType.LIMIT_EXCESS_RISK: ("stress",),
            RecommendationType.MONITOR: ("partial_evidence",),
            RecommendationType.NO_ACTION: ("no_material_issue",),
        }[recommendation_type]
    ...
    def _has_concentration_breach(
        self, request: TreasuryDecisionRequest, config: DecisionConfig
    ) -> bool:
    ...
    def _coerce_config(self, config: Any) -> DecisionConfig:
        ...
        return DecisionConfig(
            ...
            version="1.0",
            name="Default",
            enabled_recommendation_types=tuple(RecommendationType.__members__.keys()),
            recommendation_thresholds={"buy": Decimal("0.65"), "sell": Decimal("0.3")},
            priority_thresholds={"warning": Decimal("0.5"), "blocking": Decimal("0.8")},
            factor_weights={"relative_value": Decimal("0.8")},
            materiality_thresholds={"concentration": Decimal("0.2")},
            confidence_bands={"high": Decimal("0.8")},
        )
