...
class DecisionAnalytics:
    ...
    def build_explanation(self, conclusion: str, factors: list[tuple[str, Decimal | str]]) -> Explanation:
        if not conclusion:
        ...
        explanation_factors = [
            ExplanationFactor(
                name=name,
                value=Decimal(str(value)) if not isinstance(value, str) or self._is_decimal_string(str(value)) else Decimal("0"),
                direction="positive",
    ...
    def summarize(self, recommendations: tuple[Recommendation, ...]) -> dict[str, Any]:
        ...
        count_by_type = Counter(
            getattr(getattr(item, "recommendation", None), "value", str(getattr(item, "recommendation", item)))
        ...
        affected_issuers = len({asset for item in recommendations for asset in getattr(item, "affected_assets", ())})
        ...
        expected_collateral_improvement = sum(
            getattr(getattr(item, "expected_impact", None), "collateral_capacity_impact", Decimal("0"))
            for item in recommendations
        )
        policy_failure_distribution = Counter(
            str(getattr(getattr(item, "policy_summary", {}), "get", lambda *_args, **_kwargs: "")("status", ""))
            for item in recommendations
        )
        confidence_distribution = Counter(str(getattr(item, "confidence", Decimal("0"))) for item in recommendations)
        rejected_alternatives = sum(len(getattr(item, "rejected_alternatives", ())) for item in recommendations)
        no_action_ratio = Decimal("0") if not recommendations else Decimal(
            sum(1 for item in recommendations if str(getattr(getattr(item, "recommendation", None), "value", getattr(item, "recommendation", item))).upper() == "NO_ACTION")
