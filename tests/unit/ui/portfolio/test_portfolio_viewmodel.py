...
def test_viewmodel_is_immutable() -> None:
    summary = PortfolioSummary(portfolio_name="P", valuation_date="2026-07-29", market_value="1", book_value="1", total_positions=1, weighted_yield="1%", modified_duration="1", hqla_percent="50%", mil_eligible_percent="50%")
    row = PortfolioRow(isin="A", issuer="Issuer", instrument="Instrument", currency="USD", nominal="1", market_value="1", book_value="1", yield_value="1%", modified_duration="1", classification="Govt", hqla_status="Eligible", mil_status="Eligible", recommendation="Hold")
    view_model = PortfolioViewModel(summary=summary, rows=(row,), filters={"currency": "USD"}, selected_isin="A", theme="light", status="loaded", warnings=("warn",), calculation_id="calc", correlation_id="corr")
