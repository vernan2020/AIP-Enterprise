...
def _build_provider(tmp_path: Path, *, data_cutoff_date: date = date(2026, 7, 29), allow_prior: bool = False) -> ConfiguredPortfolioProvider:
    config = DemoConfig(execution_mode="CONFIGURED", demo_mode_enabled=False, data_cutoff_date=data_cutoff_date)
    source_config = ConfiguredSourceConfig(
        ...
        metadata={"allow_prior_source_date": allow_prior, "data_cutoff_date": data_cutoff_date.isoformat()},
    )
...
def test_discovery_selects_exact_pipca_txt_vector(tmp_path: Path) -> None:
    ...
    (root / "vector" / "VectorPiPCA_20260729.txt").write_text("row_id;instrument_id;price;valuation_date\n1;ABC;1000.25;2026-07-29\n", encoding="utf-8")
...
def test_discovery_excludes_sugef_and_other_unrelated_files(tmp_path: Path) -> None:
    ...
    (root / "vector").mkdir(parents=True)
    (root / "vector" / "COOPEALIANZA_SUGEF_20260731.xls").write_text("x")
    (root / "vector" / "VectorPiPCA_20260728.txt").write_text("row_id;instrument_id;price;valuation_date\n1;ABC;1000.25;2026-07-28\n", encoding="utf-8")
...
def test_discovery_rejects_future_pipca_vectors(tmp_path: Path) -> None:
    root = tmp_path / "institutional" / "Inversiones" / "2026"
    (root / "vector").mkdir(parents=True)
    (root / "vector" / "VectorPiPCA_20260801.txt").write_text("row_id;instrument_id;price;valuation_date\n1;ABC;1000.25;2026-08-01\n", encoding="utf-8")
...
def test_pipca_txt_reader_parses_semicolon_delimited_rows(tmp_path: Path) -> None:
    ...
    path.write_text("row_id;instrument_id;price;valuation_date\nA-1;ABC;1.250,40;2026-07-29\n", encoding="cp1252")
...
def test_environment_loader_reads_vector_patterns_and_extensions(monkeypatch: pytest.MonkeyPatch) -> None:
