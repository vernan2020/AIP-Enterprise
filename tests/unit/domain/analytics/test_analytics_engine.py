...
def test_normalizers_cover_constant_empty_and_single_value_series() -> None:
    ...
    assert percentile.normalize([Decimal("10"), Decimal("20"), Decimal("30")]) == [
...
def test_target_is_best_and_composite_scoring() -> None:
    ...
    composite = CompositeScore(
        ...
        score_scale=(Decimal("0"), Decimal("1")),
        score_bands=bands,
        weighted_scores=(),
    )
...
def test_ranking_engine_ties_and_deterministic_ordering() -> None:
    ...
    with pytest.raises(DuplicateRankItemError):
        engine.rank(
            [
                RankItem(business_id="A", primary_score=Decimal("1")),
                RankItem(business_id="A", primary_score=Decimal("2")),
            ],
    ...
    tie_breaker = TieBreaker(
        metric_name="volatility", business_id_first=False, ranking_order=RankingOrder.ASCENDING
    )
    ranked = engine.rank(
        [
            RankItem(
                business_id="C",
                primary_score=Decimal("10"),
                secondary_metrics=(("volatility", Decimal("2")),),
            ),
            RankItem(
                business_id="A",
                primary_score=Decimal("10"),
                secondary_metrics=(("volatility", Decimal("1")),),
            ),
        ],
        RankingOrder.DESCENDING,
    ...
    ascending_result = engine.rank(
        [
            RankItem(business_id="A", primary_score=Decimal("10")),
            RankItem(business_id="B", primary_score=Decimal("5")),
        ],
...
def test_descriptive_and_weighted_statistics() -> None:
    stats = DescriptiveStatistics(
        [Decimal("1"), Decimal("2"), Decimal("3"), Decimal("4")], sample=True
    )
    ...
    assert stats.variance() == Decimal(
        "1.6666666666666666666666666667"
    ) or stats.variance() == Decimal("1.666666666666666666666666667")
    ...
    weighted = WeightedStatistics(
        [Decimal("1"), Decimal("2"), Decimal("3")], [Decimal("1"), Decimal("1"), Decimal("2")]
    )
...
def test_normalization_edge_cases_and_tie_behaviors() -> None:
    ...
    assert percentile.normalize([Decimal("1"), Decimal("1"), Decimal("2")])[0] == Decimal(
...
def test_outlier_detection_methods_and_explanation_builder() -> None:
    ...
    assert z_detector.detect(
        [Decimal("1"), Decimal("2"), Decimal("3"), Decimal("100")]
    ).outliers == (Decimal("100"),)
    modified = OutlierDetection(OutlierMethod.MODIFIED_Z_SCORE)
    assert modified.detect([Decimal("1"), Decimal("2"), Decimal("3"), Decimal("100")]).outliers == (
        Decimal("100"),
    )
    ...
    explanation = builder.build(
        ...
        factors=[
            ExplanationFactor(
                name="value", value=Decimal("0.8"), direction="up", contribution=Decimal("0.2")
            )
        ],
    )
...
def test_weighted_statistics_and_tie_breaker_edge_cases() -> None:
    ...
    assert (
        tie_breaker.resolve(
            RankItem(business_id="A", primary_score=Decimal("1")),
            RankItem(business_id="B", primary_score=Decimal("1")),
        )
        == -1
    )
    assert (
