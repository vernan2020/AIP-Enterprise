...
from aip.platform.security.events.security_events import AuthenticationFailed, AuthenticationSucceeded, SecurityEventPublisher
from aip.platform.security.exceptions.security_exceptions import AuthenticationError, AuthorizationError, SecretProviderError, SessionError
...
from aip.platform.security.secrets.secret_provider import EnvironmentSecretProvider, InMemorySecretProvider
...
def test_identity_and_principal_are_immutable() -> None:
    identity = Identity(subject="user-1", username="alice", groups=("admins",), claims={"tenant": "t1"})
...
def test_session_manager_lifecycle_and_expiration() -> None:
    ...
    expired_token = Token(value="expired", expires_at=datetime.now(timezone.utc) - timedelta(seconds=5))
...
def test_authorization_service_and_permission_evaluator() -> None:
    evaluator = PermissionEvaluator(
        policies=(AuthorizationPolicy(name="admin-policy", evaluator=lambda role, permission: role.name == "admin" and permission.name == "read"),)
    )
    service = AuthorizationService(evaluator)
    principal = Principal(identity=Identity(subject="user-1", username="alice"), roles=("admin",), permissions=("read",))
...
def test_roles_and_permissions_support_inheritance_and_deny_override() -> None:
    ...
    principal = Principal(identity=Identity(subject="user-1", username="alice"), roles=("manager",), permissions=("approve",))
...
def test_audit_monitoring_telemetry_and_events() -> None:
    ...
    health = SecurityHealth(successful_logins=1, failed_logins=2, authorization_failures=3, active_sessions=4, expired_sessions=5, permission_checks=6)
    assert health.successful_logins == 1

    metrics = SecurityMetrics(successful_logins=1, failed_logins=2, authorization_failures=3, active_sessions=4, expired_sessions=5, permission_checks=6)
...
def test_security_config_and_exceptions() -> None:
    config = SecurityConfig(session_ttl_seconds=60, issuer_name="demo", token_ttl_seconds=120, allowed_roles=("admin", "user"))
