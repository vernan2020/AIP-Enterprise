...
def test_deterministic_chain_yields_same_results_for_repeat_runs(
    ...
    first_stress = stress_engine.evaluate(
        StressRequest(
            portfolio_reference="portfolio-1",
            gap_result=first_gap,
            projection_result=first_projection,
            configuration=stress_config,
        )
    )

    second_projection = engine.project(projection_request)
    second_gap = gap_engine.project(
        GapRequest(
            valuation_date=projection_request.valuation_date,
            cashflow_request=projection_request,
            currency="USD",
    ...
    second_stress = stress_engine.evaluate(
        StressRequest(
            portfolio_reference="portfolio-1",
            gap_result=second_gap,
    ...
    assert (
        first_stress.scenario_results[0].stressed_gap
        == second_stress.scenario_results[0].stressed_gap
    )
